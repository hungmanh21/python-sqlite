...
from quilldb.constants import OFF_RESERVED, OFF_SCHEMA_COOKIE, PAGE_SIZE, RESERVED_LEN
...
def test_memory_pager_is_a_valid_one_page_database() -> None:
    pager = Pager.memory()
    assert pager.page_count == 1
    raw = pager.read_page(1)
    assert raw[page_header_offset(1)] == 13  # PageType.LEAF_TABLE
    pager.close()  # must not touch disk or raise despite no file descriptor


def test_memory_pager_creates_no_filesystem_entry(tmp_path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    Pager.memory().close()
    assert list(tmp_path.iterdir()) == []


def test_bump_schema_cookie_increments_and_persists(tmp_path) -> None:
    path = tmp_path / "test.db"
    pager = Pager.create(path)
    assert pager.bump_schema_cookie() == 1
    assert pager.bump_schema_cookie() == 2
    pager.close()

    raw_header = path.read_bytes()[:100]
    cookie = int.from_bytes(raw_header[OFF_SCHEMA_COOKIE : OFF_SCHEMA_COOKIE + 4], "big")
    assert cookie == 2
